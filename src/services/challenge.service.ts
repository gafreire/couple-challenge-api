...
export const challengeService = {
  async createChallenge(
    ...
    const hasActiveChallenge =
      await challengeRepository.coupleHasActiveChallenge(couple.id);
    if (hasActiveChallenge)
      throw new BadRequestError("You already have an active challenge");
    const validPeriods = ["mensal", "trimestral", "semestral", "anual"];
    if (!validPeriods.includes(data.period_type))
      throw new BadRequestError(
        "Invalid period type. Must be: mensal, trimestral, semestral, or anual",
      );
  ...
  async getActiveChallenge(userId: string) {
    ...
    if (couple.status !== "active")
      throw new BadRequestError("You don't have an active couple");
    const activeChallenge = await challengeRepository.findActiveByCouple(
      couple.id,
    );
    if (!activeChallenge)
      throw new NotFoundError("You don't have an active challenge");
  ...
  async finishChallenge(userId: string, challengeId: string) {
    ...
    if (challenge.couple_id !== couple.id)
      throw new BadRequestError(
        "This challenge does not belong to your couple",
      );
    if (challenge.status !== "active")
      throw new BadRequestError("Only active challenges can be finished");

    const calculatedPoints =
      await taskCompletionRepository.calcChallengeScore(challengeId);
    if (!calculatedPoints)
      throw new NotFoundError("No task completions found for this challenge");
    ...
    if (calculatedPoints.user_id_1_score > calculatedPoints.user_id_2_score)
      ...
      calculatedPoints.user_id_2_score > calculatedPoints.user_id_1_score
    )
      ...
      if (calculatedPoints.user_id_1_tasks > calculatedPoints.user_id_2_tasks) {
        ...
        calculatedPoints.user_id_2_tasks > calculatedPoints.user_id_1_tasks
      ) {
        winner = couple.user_id_2;
      }
  ...
  async getChallengeScore(userId: string, challengeId: string) {
    const couple = await coupleRepository.findByUserId(userId);
    if (!couple) throw new NotFoundError("You don't have a couple");
    ...
    const score =
      await taskCompletionRepository.calcChallengeScore(challengeId);
    if (!score) throw new NotFoundError("No score found for this challenge");
